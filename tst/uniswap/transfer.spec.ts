...
describe("Uniswap V3 Events", () => {
    ...
    it("is parsing the right Burn event with same owner and sender", () => {
        const UniswapTransferEventHandler = new UniswapV3EventHandler();
        const result =  UniswapTransferEventHandler.getEventsToRegister()[3].eventHandler({
        ...
        expect(result[0].message).toBe('Your liquidity of the amount 12345 has been minted');
    });
    ...
    it("is parsing the right Burn event with different owner and sender", () => {
        ...
        expect(result[0].message).toBe('ABC minted liquidity for you and a total of 12345');
    });
